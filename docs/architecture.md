...
- models/local_qwen.py：量化加载、仅解码新增词元，以及流式生成失败时如何把异常传回调用方。
...
标题走另一条规则：session_title() 把所有空白压成单行，**不参与转义**。
标题取自用户问题的前若干字，而提问框允许换行；`# 标题` 后面若跟着
`## 助手` 这样的整行内容，会被 parse_session 解析成一条真实的助手消息，
既写进记录，也会作为历史进入下一轮提示词。压成单行后标题无法撑破这一行，
而问题原文仍完整保存在第一条用户消息里。
...
每个模型组件（Embedder、Generator、VisionGenerator、Reranker）各自持有一把锁，
load() 与 release() 都在锁内执行：并发的第二次调用会等第一次加载完再复用同一个实例，
而不是各加载一份。serve --warmup 起的是后台线程，与首个请求几乎同时进入加载路径，
这正是加锁要挡住的场景——12G 显存上重复加载一份 8B 权重会直接 OOM。
锁是组件级而不是全局的，组件之间不存在嵌套加锁，因此不会与 Serving 层的串行锁互相等待。
